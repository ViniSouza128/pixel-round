...
\newcommand{\shijian}[1]{\textnormal{\footnotesize\color{bg!90!white}\textsf{#1}}}
...
\begin{titlepage}
  ...
  \begin{minipage}{0.78\textwidth}
    ...
    {\sffamily\footnotesize\color{muted}由 Pixel Round 体素化的直径 12 球。}
  \end{minipage}
  \vspace{1.0cm}\\
  {\color{rule}\rule{0.6\textwidth}{0.6pt}\par}
  \vspace{1.0cm}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{\emph{阈值}}
\end{subfigure}
\caption{同一直径 20 的圆在三种算法中的表现。直径较大时差异更细微，直径较小时差异更明显。}
\label{fig:comp-d20}
\end{figure}
...
\end{subfigure}
\caption{三维体素化。每个立方体是一个体素，其中心坐标根据隐式方程测试。}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{Y 方向切割 50\%（$y \geq 5$ 被舍弃）}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig10_3d_cut_x50.png}
  \caption{X 方向切割 50\%（$x \geq 10$ 被舍弃）}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig11_3d_cut_diag50.png}
  \caption{对角切割（$x + y \geq 15$ 被舍弃）}
\end{subfigure}
\caption{同一椭球（$W = 20, H = 10, D = 12$）上的三种平面切割，每种保留 50\% 的体积。请注意对角切割的阶梯形图案。}
\label{fig:cortes}
\end{figure}
