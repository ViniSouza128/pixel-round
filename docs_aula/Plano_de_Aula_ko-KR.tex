...
\newcommand{\sigan}[1]{\textnormal{\footnotesize\color{bg!90!white}\textsf{#1}}}
...
\begin{titlepage}
  ...
  \begin{minipage}{0.78\textwidth}
    ...
    {\sffamily\footnotesize\color{muted}Pixel Round로 복셀화된 지름 12 구.}
  \end{minipage}
  \vspace{1.0cm}\\
  {\color{rule}\rule{0.6\textwidth}{0.6pt}\par}
  \vspace{1.0cm}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{\emph{임계값}}
\end{subfigure}
\caption{세 알고리즘에서 같은 지름 20 원의 비교. 지름이 클수록 차이는 미묘하며, 작은 지름에서 더 뚜렷하다.}
\label{fig:comp-d20}
\end{figure}
...
\begin{subfigure}[t]{0.42\linewidth}\centering
  ...
  \caption{구 $D = 12$ ($r_x = r_y = r_z = 6$).}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.42\linewidth}\centering
  \includegraphics[width=\linewidth]{fig08_3d_ellipsoid.png}
  \caption{타원체 $W = 20, H = 10, D = 12$.}
\end{subfigure}
\caption{3D 복셀화. 각 정육면체가 하나의 복셀이며, 중심 좌표가 음함수 방정식으로 판정된다.}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{Y 절단 50\% ($y \geq 5$ 제거)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig10_3d_cut_x50.png}
  \caption{X 절단 50\% ($x \geq 10$ 제거)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig11_3d_cut_diag50.png}
  \caption{대각 절단 ($x + y \geq 15$ 제거)}
\end{subfigure}
\caption{같은 타원체($W = 20, H = 10, D = 12$)에 대한 세 가지 평면 절단, 각 부피의 50\%를 유지. 대각 절단의 계단 패턴에 주목.}
\label{fig:cortes}
\end{figure}
