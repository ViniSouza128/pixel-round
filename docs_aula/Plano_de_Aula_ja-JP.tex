...
\newcommand{\jikan}[1]{\textnormal{\footnotesize\color{bg!90!white}\textsf{#1}}}
...
\begin{titlepage}
  ...
  \begin{minipage}{0.78\textwidth}
    ...
    {\sffamily\footnotesize\color{muted}Pixel Round によりボクセル化された直径 12 の球。}
  \end{minipage}
  \vspace{1.0cm}\\
  {\color{rule}\rule{0.6\textwidth}{0.6pt}\par}
  \vspace{1.0cm}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{\emph{閾値}}
\end{subfigure}
\caption{同じ直径 20 の円を 3 つのアルゴリズムで比較。直径が大きいほど違いは微妙になり，小さい直径では明確になる。}
\label{fig:comp-d20}
\end{figure}
...
\begin{subfigure}[t]{0.31\linewidth}\centering
  ...
  \caption{Y 軸 50\% 切断（$y \geq 5$ を破棄）}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig10_3d_cut_x50.png}
  \caption{X 軸 50\% 切断（$x \geq 10$ を破棄）}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\linewidth}\centering
  \includegraphics[width=\linewidth]{fig11_3d_cut_diag50.png}
  \caption{対角切断（$x + y \geq 15$ を破棄）}
\end{subfigure}
\caption{同じ楕円体（$W = 20, H = 10, D = 12$）に対する 3 つの平面切断，それぞれ体積の 50\% を保持。対角切断の階段状のパターンに注目。}
\label{fig:cortes}
\end{figure}
